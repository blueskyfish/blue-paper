      ...
      return new Promise<void>((resolve, reject) => {
        this._connection.beginTransaction((err: MysqlError) => {
          if (err) {
            this.log.warn(
              DB_GROUP,
              `Begin Transaction Error: ${err.errno} -> ${err.message}`
            );
    ...
    return new Promise<boolean>((resolve) => {
      this._connection.commit((err: MysqlError) => {
        if (err) {
          this.log.warn(
            DB_GROUP,
            `Commit Error: ${err.errno} -> ${err.message}`
          );
    ...
    return new Promise<boolean>((resolve) => {
      this._connection.rollback((err: MysqlError) => {
        if (err) {
          this.log.warn(
            DB_GROUP,
            `Rollback Error: ${err.errno} -> ${err.message}`
          );
          return resolve(false);
        }
        resolve(true);
    ...
    return new Promise<any>((resolve, reject) => {
      connection.query(sql, values, (err: MysqlError, result) => {
        if (err) {
          this.log.warn(
            DB_GROUP,
            `Action (${action}) Error: ${err.errno} -> ${err.message}`
          );
          this.log.warn(DB_GROUP, `Sql:\n${err.sql}`);
      ...
      this._pool.getConnection(
        (err: MysqlError, connection: PoolConnection) => {
          if (err) {
            this.log.error(
              DB_GROUP,
              `Conn Error: ${err.errno} -> ${err.message}`
            );
